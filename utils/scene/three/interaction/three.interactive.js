...
var c = class {
  ...
  onMouseClick = e => {
    this.update();
    let s = new i("click", e);
    const deltaTime = Date.now() - this.startDate;
    const deltaMove = (new THREE.Vector2(this.mouse.x - this.startMouse.x, this.mouse.y - this.startMouse.y)).length();
    if (deltaTime > 250 || deltaMove > 0.15) return;
